...
namespace SelfHostWebApiRelay.api
{
    ...
    public class RelayController : ApiController
    {
        ...
        private static readonly Uri _service = new Uri(AppBag.Data.Value.RelayAddress);
        ...
        {
            var relayAddress = new Uri(_service.AbsoluteUri.AppendPathSegment(id.ToString(CultureInfo.InvariantCulture)));

            // Submit request to our service which we relay. We only ask for the headers first so that we don't read the entire response just yet.
            using (HttpResponseMessage serviceResponse = await _client.GetAsync(relayAddress, HttpCompletionOption.ResponseHeadersRead))
            {
                // Return response
                return CreateResponse(serviceResponse);
            }
