...
{
    [TestClass]
    public class FullContactTest
    {
        public FullContactTest()
        {
            configMgr = new ConfigMgr();
            FullContactAPIKey = configMgr.GetAppSetting("fc:X-FullContact-APIKey");
        }

        private ConfigMgr configMgr { get; set; }
        private string FullContactAPIKey { get; set; }


        [TestMethod]
        public void TestUrl()
        {
            var targetUrl = getFullContactPersonByEmailUrl("dustin.lawler@datadoghq.com");
            Assert.AreEqual("https://api.fullcontact.com/v2/person.json?email=dustin.lawler%40datadoghq.com", targetUrl);
        }

        [TestMethod]
        public void TestFlurlGetPersonByEmail()
        {
        ...
        private string getFullContactPersonByEmailUrl(string email)
        {
            var url = new ApiUrl
            {
        ...
        private bool TryGetPersonByEmailResult(string email, out JObject o)
        {
            try
            {
                Url targetUrl = getFullContactPersonByEmailUrl(email);
                var call = targetUrl.WithHeader("X-FullContact-APIKey", FullContactAPIKey).GetAsync();
