...
{
    public static class JsonSerializationExtensionBase
    {
        ...
        {
            Serialize(((Stream)stream), obj, settings, encodingType, knownTypes);
        }
        ...
        public static T Deserialize<T>(this FileStream stream,
            ...
            IEnumerable<Type> knownTypes = null) where T : class
        {
            return Deserialize<T>(((Stream)stream), settings, knownTypes);
        }
            ...
            IEnumerable<Type> knownTypes = null) where T : class
        {
            ...
            Serialize(ms, obj, settings, encodingType, knownTypes);
            return ms;
        }
        ...
        IEnumerable<Type> knownTypes = null) where T : class
        {
            ...
            {
                return Deserialize<T>(ms, settings, knownTypes);
            }
        }
        ...
        IEnumerable<Type> knownTypes = null) where T : class
        {
            return Convert.ToBase64String(SerializeToBytes(obj, settings, encodingType, knownTypes));
        }
        ...
        IEnumerable<Type> knownTypes = null) where T : class
        {
            return DeserializeBytes<T>(Convert.FromBase64String(value), settings, encodingType, knownTypes);
        }
        ...
        public static FileInfo Serialize<T>(this T obj, String filePath,
            ...
            IEnumerable<Type> knownTypes = null) where T : class
        {
            return Serialize(obj, new FileInfo(filePath), lockWaitMs, rollbackType, settings, encodingType, knownTypes);
        }
...
// ReSharper disable once AssignNullToNotNullAttribute
        ...
        IEnumerable<Type> knownTypes = null) where T : class
        {
            ...
            if (fileInfo.Directory != null && fileInfo.Directory.Exists)
            {
                ...
                using (SerializeToMemoryStream(obj, settings, encodingType, enumerable))
                {
                    using (new WriteFileAccess(fileInfo, lockWaitMs, TimeSpan.FromSeconds(30)))
                    {
                        using (var fs = fileInfo.OpenFileStream(FileMode.Create, FileAccess.Write, FileShare.None, lockWaitMs, true))
        ...
        IEnumerable<Type> knownTypes = null) where T : class
        {
            return Deserialize<T>(fileInfo, lockWaitMs, settings, knownTypes);
        }
