...
{
    public static class ClassExtensionBase
    {
            ...
            , IEnumerable<Type> ignorePropertyTypes = null)
        {
            ...
            foreach (var item in anonymousObject.GetType().GetProperties(BindingFlags.Instance | BindingFlags.Public))
            {
                ...
                else if (v.InheritsOrImplements<IList>())
                {
                    ...
                    if (list != null)
                    {
                        Type ti = t.GenericTypeArguments[0];
                        Type vi = v.GenericTypeArguments[0];
                        IList propertyList = t.CreateInstance(list.Count) as IList;

                        for (int i = 0; i < list.Count; i++)
                        {
                            if (ti.IsGenericType && (ti.GetGenericTypeDefinition() == typeof(Nullable<>)))
                            {
                                propertyList.Add(Convert.ChangeType(list[i], Nullable.GetUnderlyingType(ti)));
                            }
                            else if (ti.IsClass && !ti.Namespace.StartsWith("System"))
                            {
                                var newti = ti.CreateInstance();
                                newti.ConvertFromDynamic(list[i]);
                                propertyList.Add(newti);
                            }
                            else
                            {
                                propertyList.Add(vi == typeof(string) ? ((string)list[i]).ParseAs(ti) : Convert.ChangeType(list[i], ti));
                            }
                        }
