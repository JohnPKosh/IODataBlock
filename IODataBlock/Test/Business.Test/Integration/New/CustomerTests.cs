...
namespace Business.Test.Integration.New
{
    ...
    public class CustomerTests
    {
        ...
        public void SearchJson_Test()
        {
            ...
            var response = baseService.SearchJson(TypeName, filters, columns, scriptKey: "search");
            CheckJsonResponse(response);
        }
        ...
        public void SearchJObjectsContactsSavedSearch()
        {
            ...
            //var columns = new[] { "custentity_cr_ext_cross_id" };
            //var filters = new[] { NsSearchFilter.NewStringFilter("custentity_cr_ext_cross_id", SearchStringFieldOperatorType.Is, externalid) };
            var response = baseService.SearchJObjectsAsync(TypeName, null, null, "customsearch_cr_initial_target_contact", scriptKey: "search").Result;
            CheckJObjectsResponse(response);
        }

        [TestMethod]
        public void SearchDynamicListContactsSavedSearch()
        {
            var externalid = TestGuid;
            //var columns = new[] { "custentity_cr_ext_cross_id" };
            //var filters = new[] { NsSearchFilter.NewStringFilter("custentity_cr_ext_cross_id", SearchStringFieldOperatorType.Is, externalid) };
        ...
        [TestMethod]
        public void DeleteLEADTargets()
        {
            //var contactResponse = baseService.SearchDynamicListAsync("contact", null, null, "customsearch_cr_initial_target_contact", scriptKey: "search").Result;
            //foreach (dynamic o in contactResponse.ResponseData)
            //{
            //    ResponseObject<string, bool> deleteResponse = baseService.DeleteById(o.id, "contact");
            //    if (!deleteResponse.ResponseData)
            //    {
            //        var hasErr = deleteResponse.HasExceptions;
            //    }
            //}

            var response = baseService.SearchDynamicListAsync(TypeName, null, null, "customsearch_initialtarget", scriptKey: "search").Result;
            foreach (dynamic o in response.ResponseData)
            {
                if (o.columns.entitystatus.name == "LEAD-Initial Target")
                {
                    ResponseObject<string, bool> deleteResponse = baseService.DeleteById(o.id, "lead");
                    if (!deleteResponse.ResponseData)
                    {
                        var hasErr = deleteResponse.HasExceptions;
                    }
